...
export async function runScheduledTasks(env) {
  ...
  // (냉캐시 시 몇 초 소요)이 없게 한다. 한 번에 최대 48콜이나 쓰는 무거운 작업이라(2026-07-26
  // 쿼터 소진 사고 이후) 15분 -> 1시간으로 주기를 늘렸다 - 팀 폼/부상/배당 같은 정보는 어차피
  // 시간 단위로 바뀌지 분 단위로 바뀌진 않아서 이 정도 주기로도 충분하다.
  if (await shouldRun(env, `${KV_KEYS.lastRunPrefix}analysis`, 60 * 60 * 1000)) {
