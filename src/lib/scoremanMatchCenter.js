...
// 78139, 변함없음)로 재확인해보니 리그ID가 "121"로 바뀌어 있었다(K3로 추정되는 조합은 "120"). 하루
// 더 지난 2026-08-30엔 금산인삼FC(팀ID 78137, 역시 변함없음)가 이번엔 "105"로 또 바뀌어 있었다 -
// 팀ID는 날짜가 지나도 계속 안정적인데 리그ID는 거의 매일 바뀌는 걸 반복 확인했으니, 이 사이트의
// "리그ID"는 시즌 고정값이 아니라 그날그날 라운드/조 편성에 따라 재배정되는 값으로 보고 아예 매칭
// 조건에서 뺐다. 팀ID+킥오프 초 단위 일치만으로 매칭한다 - 두 조건이 동시에 맞아떨어지는 다른 경기가
// 우연히 존재할 가능성은 사실상 없다.
// K3/K4는 팀이 많아 한 번에 다 못 채운다 - 확인되는 경기부터 하나씩 늘린다. 2026-08-29에 그날 진행된
// K3/K4 6경기(기존 4개 + 8개 추가), 2026-08-30에 라이브 중이던 2경기(+3개, 진주시민은 상대팀인
// 금산인삼FC 매핑만으로도 이미 찾아지지만 안전하게 같이 기록)를 대조해 확인했다.
