...
// featured: AI 분석 탭에 실제 분석 카드를 만들어줄 대회만 true(나머지는 링크만 보여준다 - 팀당
// 조회 비용이 커서 전체 대회를 다 만들 수 없음). 2026-08-11 사용자 요청으로 유럽 5대리그 등 범용
...
// 이적시장 탭은 사용자 요청으로 관심 리그만 좁혀서 보여준다: 유럽 5대리그 + K리그1~2. 원래는 K리그
// 3~4/네덜란드(에레디비지에)/사우디 1부/미국(MLS)도 포함했었는데, 순환 조회 로스터가 커질수록 팀당
// 5팀/5분 예산으로 한 바퀴 도는 데 더 오래 걸려서(약 250팀일 땐 4시간 가까이) "이적시장이 느리다"는
// 체감으로 이어졌다(2026-08-31) - 관심도가 낮은 리그를 아예 빼서 로스터 자체를 줄이는 쪽으로
// 정리했다(약 절반으로 줄어 한 바퀴 도는 시간도 그만큼 짧아짐, API 호출도 그만큼 아낌). 나머지
// 리그(포르투갈/챔피언십/브라질/일본/노르딕 등과 함께 이번에 빠진 것들)는 이적시장 탭 대상에서만
// 빠지고 경기/순위 탭에는 그대로 나온다.
const TRANSFER_MARKET_LEAGUE_CODES = ["PL", "PD", "BL1", "SA", "FL1", "KL1", "KL2"];
