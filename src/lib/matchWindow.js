...
// config.js에 realtime: false로 표시해둔 대회(사용자가 실시간 추적에서 빼기로 확인, 2026-09-17 -
// 노르웨이/덴마크/스코틀랜드/호주/사우디/중국/J2/J3/브라질)도 같은 이유로 제외한다 - 그 대회에서
// 경기가 열려도 "활성 시간대"로 안 쳐서, K리그·유럽 5대리그처럼 실제로 보는 대회에 API-Football
// 분당 한도 여유가 더 가게 한다.
const ACTIVITY_CHECK_EXCLUDED_CODES = new Set([
  "FRIENDLY",
  "INTFRIENDLY",
  ...COMPETITIONS.filter((c) => c.realtime === false).map((c) => c.code),
]);
