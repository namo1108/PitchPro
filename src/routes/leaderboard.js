...
import { KV_KEYS, LEADERBOARD_CACHE_TTL_SECONDS, LEADERBOARD_SIZE, GOAT_USERNAMES } from "../lib/config.js";
import { getAuthedUser, levelProgress, normalizeUsername } from "../lib/auth.js";
...
// GOAT_USERNAMES(운영자 이스터에그)는 순위/메달 경쟁에서 완전히 빼고 맨 위에 별도로 고정 노출한다.
async function computeLeaderboard(env) {
  ...
  const goatSet = new Set(GOAT_USERNAMES.map(normalizeUsername));

  const ranked = users
    .filter(Boolean)
    .filter((u) => !goatSet.has(normalizeUsername(u.username)))
  ...
  const goats = users
    .filter(Boolean)
    .filter((u) => goatSet.has(normalizeUsername(u.username)))
    .map((u) => ({ username: u.username, nickname: u.nickname, points: u.points || 0, title: levelProgress(u.points || 0, u.username).title }));
...
export async function handleLeaderboard(request, env) {
  ...
  let { ranked, goats } = cached || {};
  if (!ranked) {
    ({ ranked, goats } = await computeLeaderboard(env));
    await putJSON(env, KV_KEYS.leaderboardCache, { ranked, goats }, { expirationTtl: LEADERBOARD_CACHE_TTL_SECONDS });
  }
  ...
  const withRelation = ({ username, ...rest }) => ({
    ...rest,
  ...
  const publicEntries = ranked.map(withRelation);
  const publicGoats = goats.map(withRelation);

  const meProgress = user ? levelProgress(user.points || 0, user.username) : null;
  const meEntry = ranked.find((e) => e.username === user?.username);
  const me = user ? { nickname: user.nickname, points: user.points || 0, rank: meEntry?.rank ?? null, progress: meProgress } : null;

  return json({ entries: publicEntries, goats: publicGoats, me });
}
